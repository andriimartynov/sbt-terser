...
object Import {
  ...
  val terser: TaskKey[Pipeline.Stage] = TaskKey[Pipeline.Stage]("terser", "Perform terser optimization on the asset pipeline.")

  case class OpGrouping(inputFiles: Seq[PathMapping], outputFile: String)

  object TerserKeys {
    val terserAppDir: SettingKey[File] =
      SettingKey[File]("terser-app-dir", "The top level directory that contains your app js files. In effect, this is the source folder that terser reads from.")
    val terserBuildDir: SettingKey[File] =
      SettingKey[File]("terser-build-dir", "By default, all modules are located relative to this path. In effect this is the target directory for terser.")
    ...
    val terserWrap: SettingKey[Option[String]] =
      settingKey[Option[String]]("Embed everything in a big function, making the “exports” and “global” variables available. Default: None")
  }
