...
import com.typesafe.sbt.web.{Compat, SbtWeb, incremental}
import sbt.Keys._
import sbt._
...
object SbtTerser extends AutoPlugin {
  ...
  private def runOptimizer: Def.Initialize[Task[Pipeline.Stage]] = Def.task {
    ...
    lazy val beautifyOptionsArgs: Seq[String] =
      terserBeautifyOptions
        .value
        .fold(Seq.empty[String]) {
          options => Seq("-b", options)
        }
    ...
    lazy val compressOptionsArgs: Seq[String] =
      terserCompressOptions
        .value
        .fold(Seq.empty[String]) {
          options => Seq("-c", options)}

    lazy val configFileArgs: Seq[String] =
      terserConfigFile
        .value
        .fold(Seq.empty[String]) {
          file => Seq("--config-file", file.getPath)}

    lazy val definitionsOptionsArgs: Seq[String] =
      terserDefinitionsOptions
        .value
        .fold(Seq.empty[String]) {
          options => Seq("-d", options)}

    lazy val ecmaArgs: Seq[String] =
      terserEcma
        .value
        .fold(Seq.empty[String]) {
          options => Seq("--ecma", options)}
    ...
    lazy val mangleOptionsArgs: Seq[String] =
      terserMangleOptions
        .value
        .fold(Seq.empty[String]) {
          options => Seq("-m", options)}

    lazy val manglePropsArgs: Seq[String] =
      terserMangleProps
        .value
        .fold(Seq.empty[String]) {
          options => Seq("--mangle-props", options)}
    ...
    lazy val nameCacheFileArgs: Seq[String] =
      terserNameCacheFile
        .value
        .fold(Seq.empty[String]) {
          file => Seq("--name-cache", file.getPath)}
            ...
            modifiedGroupings =>
              ...
              if (modifiedGroupings.nonEmpty) {
                ...
                val res: Seq[(OpGrouping, OpResult)] = modifiedGroupings.map(op => {
                  ...
                  (op, Try(execute(arg))
                    .map[OpResult](_ =>
                    OpSuccess(op.inputFiles.map(_._1).toSet, Set(outputFile))
                  ).recover { case _ => OpFailure }.get)
                })
  ...
  private def syncMappings(
                            ...
                            target: File): File = SbtWeb.syncMappings(
